...
func (s *style) getStyle(jsName string) string {
	...
	var value string
	for _, declaration := range strings.Split(attr, ";") {
		name, candidate, ok := strings.Cut(declaration, ":")
		if !ok {
			continue
		}
		if strings.EqualFold(strings.TrimSpace(name), cssName) {
			// CSSStyleDeclaration exposes the value separately from its priority.
			// Keep scanning because CSS uses the last valid declaration.
			value = cssImportant.ReplaceAllString(strings.TrimSpace(candidate), "")
...
func newDOMParser(opts ...engineOption) *domParser {
	p := &domParser{
		options: defaultOpts(),
	}
